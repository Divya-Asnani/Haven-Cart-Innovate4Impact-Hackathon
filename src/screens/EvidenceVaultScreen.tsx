...
import { ArrowLeft, Plus, FileText, Camera, Mic, Trash2, ShieldCheck, Clock, X } from 'lucide-react-native';
import { COLORS } from '../constants/theme';
import { useApp } from '../context/AppContext';
import { getEvidenceQueue, deleteEvidence, getDecryptedEvidencePayload, EvidenceQueueItem, syncOfflineEvidence } from '../storage/evidenceQueue';
...
export const EvidenceVaultScreen = ({ navigation }: { navigation: any }) => {
  ...
  const [viewModalItem, setViewModalItem] = useState<{ item: EvidenceQueueItem; payload: string } | null>(null);
  ...
  useEffect(() => {
    if (isFocused) {
      loadQueue();
      syncOfflineEvidence().then(() => loadQueue()).catch(() => {});
    }
  }, [isFocused]);
  ...
  const handleView = async (item: EvidenceQueueItem) => {
    triggerTouchActivity();
    try {
      const payload = await getDecryptedEvidencePayload(item.evidence_id);
      if (!payload) {
        Alert.alert('Evidence Unavailable', 'Could not read encrypted evidence payload.');
        return;
      }
      setViewModalItem({ item, payload });
    } catch (err: any) {
      Alert.alert('Error', err.message || 'Failed to decrypt evidence.');
    }
  };
  ...
  const handleDelete = (evidenceId: string) => {
    triggerTouchActivity();
    if (Platform.OS === 'web') {
      if (typeof window !== 'undefined' && window.confirm('Are you sure you want to delete this securely stored document? This cannot be undone.')) {
        deleteEvidence(evidenceId).then(() => loadQueue());
      }
      return;
    }
    Alert.alert(
            ...
            <View style={{ gap: 12 }}>
              {evidenceList.map(item => (
                <View key={item.evidence_id} style={{ backgroundColor: COLORS.card, padding: 16, borderRadius: 16, borderWidth: 1, borderColor: COLORS.border, flexDirection: 'row', alignItems: 'center' }}>
                  <TouchableOpacity onPress={() => handleView(item)} style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <View style={{ flex: 1, marginLeft: 12 }}>
                      <Text style={{ fontSize: 14, fontWeight: '700', color: COLORS.text, marginBottom: 4 }}>
                        {item.type.charAt(0) + item.type.slice(1).toLowerCase()} Record (Tap to View)
                      </Text>
        ...
        {viewModalItem && (
          ...
          >
            <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 20 }}>
              <View style={{ width: '100%', maxWidth: 450, backgroundColor: COLORS.surface, borderRadius: 16, padding: 20, borderWidth: 1, borderColor: COLORS.border }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  <Text style={{ fontSize: 16, fontWeight: '800', color: COLORS.text }}>
                    Decrypted {viewModalItem.item.type.charAt(0) + viewModalItem.item.type.slice(1).toLowerCase()} Record
                  </Text>
                  <TouchableOpacity onPress={() => setViewModalItem(null)} style={{ padding: 4 }}>
                    <X size={20} color={COLORS.textMuted} />
                  </TouchableOpacity>
                </View>
                
                <Text style={{ fontSize: 12, color: COLORS.textMuted, marginBottom: 12 }}>
                  Captured: {new Date(viewModalItem.item.created_at).toLocaleString()}
                </Text>

                <ScrollView style={{ maxHeight: 300, backgroundColor: COLORS.background, padding: 12, borderRadius: 8, borderWidth: 1, borderColor: COLORS.border, marginBottom: 16 }}>
                  {viewModalItem.item.type === 'TEXT' ? (
                    <Text style={{ fontSize: 14, color: COLORS.text, lineHeight: 20 }}>{viewModalItem.payload}</Text>
                  ) : (
                    <Text style={{ fontSize: 13, color: COLORS.textSecondary, fontStyle: 'italic' }}>
                      {viewModalItem.item.type === 'PHOTO' ? '📷 Encrypted Photo Payload' : '🎙️ Encrypted Voice Recording Payload'}{'\n'}
                      Size: {Math.round((viewModalItem.payload.length * 0.75) / 1024)} KB binary payload decrypted safely.
                    </Text>
                  )}
                </ScrollView>

                <TouchableOpacity
                  onPress={() => setViewModalItem(null)}
